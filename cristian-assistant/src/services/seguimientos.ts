...
import type { SeguimientoPendiente } from "@/types/entities";

export async function getSeguimientosPendientes(): Promise<SeguimientoPendiente[]> {
...
export async function crearSeguimientosParaPresupuesto(
  ...
  const rows = [
    ...
    { presupuesto_id: presupuestoId, tipo: "pre_vencimiento", programado_para: preVen.toISOString(), canal },
  ];
