    ...
    <Card className="auth-card animate-auth-card-in w-full max-w-[calc(100vw-2rem)] border-border/60 bg-card/95 shadow-xl backdrop-blur-sm rounded-xl md:max-w-md md:rounded-2xl">
      ...
      <CardContent className="space-y-5 pb-6 pt-0 safe-area-inset-bottom md:pb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {denied && (
            <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 px-4 py-3 text-sm transition-colors dark:border-amber-400/30 dark:bg-amber-500/15" role="alert">
              ...
              <p className="mt-1 text-amber-600 dark:text-amber-400/90">{t.deniedText}</p>
            </div>
          )}
          {!denied && error && (
        ...
        <div className="auth-separator">
          <span className="mx-3 text-[0.65rem] font-medium uppercase tracking-widest text-muted-foreground/60">
            {t.orWith}
          </span>
        </div>
        ...
        <div className="flex justify-center gap-4">
          <Button
            ...
            variant="outline"
            size="icon"
            className="auth-oauth-btn h-11 w-11 rounded-full border-border/60 bg-muted/20 transition-all duration-300 hover:scale-110 hover:border-primary/40 hover:bg-accent/80 focus:ring-2 focus:ring-primary focus:ring-offset-2"
            onClick={() => {
              supabase.auth.signInWithOAuth({
                provider: "google",
                options: { redirectTo: `${window.location.origin}/auth/callback?next=/dashboard/elegir` },
              });
            }}
            aria-label={t.google}
          >
        ...
        <p className="text-center text-xs text-muted-foreground/80 pt-4">{t.privateNote}</p>
      </CardContent>
    </Card>
  );
}
