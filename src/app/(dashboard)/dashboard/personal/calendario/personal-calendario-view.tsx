...
import {
  getEventosPersonalesMes,
  eliminarPersonalEvento,
  crearPersonalEvento,
  type PersonalEvento,
...
export function PersonalCalendarioView() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await getEventosPersonalesMes(ano, mes);
      setEventos(data);
    } catch {
      setEventos([]);
    } finally {
  ...
  async function handleDelete(id: string) {
    try {
      await eliminarPersonalEvento(id);
  ...
  async function handleCrear() {
    ...
    if (!titulo || !nuevaInicio || !nuevaFin) {
      toast.error("Completá título, inicio y fin");
      return;
    }
    setAdding(true);
    try {
      await crearPersonalEvento({
        titulo,
        fecha_inicio: new Date(nuevaInicio).toISOString(),
        fecha_fin: new Date(nuevaFin).toISOString(),
    ...
    <div className="grid gap-6 md:grid-cols-3">
          ...
          {loading ? (
            ...
            <ul className="space-y-2">
              {eventosDelDia.map((e) => (
                <li
                  key={e.id}
                  className="group flex items-start gap-2 rounded-lg border p-3 text-sm"
                >
                  <CalendarIcon className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                  <div className="min-w-0 flex-1">
                    <p className="font-medium">{e.titulo}</p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(e.fecha_inicio).toLocaleString("es-AR", {
                        day: "numeric",
                        month: "short",
                        hour: "2-digit",
                        minute: "2-digit",
                      ...
                      {" – "}
                      {new Date(e.fecha_fin).toLocaleString("es-AR", {
                        day: "numeric",
                        month: "short",
                        hour: "2-digit",
                        minute: "2-digit",
      ...
      <Card className="md:col-span-3">
        <CardHeader>
          ...
          <CardDescription>Título, fecha/hora inicio y fin</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex items-end">
              <Button
                onClick={handleCrear}
                disabled={adding || !nuevoTitulo.trim() || !nuevaInicio || !nuevaFin}
              >
