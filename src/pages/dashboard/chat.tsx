...
const CHAT_EDGE_FN = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat`;
...
export default function ChatPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      fetch(`${CHAT_EDGE_FN}/history?limit=50`, {
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const res = await fetch(CHAT_EDGE_FN, {
          method: "POST",
          headers: {
        ...
        while (true) {
          ...
          if (!added) {
            setMessages((prev) => [...prev, { id: assistantId, role: "assistant", content: captured }]);
            added = true;
          } else {
        ...
        if (!fullText.trim() && !added) {
          setMessages((prev) => [...prev, { id: assistantId, role: "assistant", content: "(Sin respuesta)" }]);
        }
      } catch (err) {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">{tp.chatTitle}</h1>
        <p className="text-muted-foreground">{tp.chatSubtitle}</p>
      </div>
      <Card className="flex flex-1 flex-col overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between border-b py-4">
          <div>
            <CardTitle className="text-lg">Conversación</CardTitle>
            <CardDescription>Escribe para interactuar. El asistente puede crear leads, consultar presupuestos y más.</CardDescription>
          </div>
            ...
            <div className="space-y-4">
              {!historyLoaded ? (
                <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-muted-foreground">
                  <Loader2 className="h-8 w-8 animate-spin" /><p>Cargando historial...</p>
                </div>
              ) : messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-muted-foreground">
                  <Bot className="h-12 w-12" />
                  <p>Hola, soy el Assistant Cristian Alancay. ¿En qué puedo ayudarte?</p>
                  <p className="text-sm">Puedes pedirme crear un lead, consultar presupuestos o listar leads.</p>
                </div>
              ) : null}
              ...
              {isLoading && !isStreaming && (
                <div className="flex gap-3">
                  ...
                  <div className="rounded-lg bg-muted px-4 py-2"><span className="animate-pulse text-muted-foreground">...</span></div>
                </div>
              )}
            </div>
          </div>
