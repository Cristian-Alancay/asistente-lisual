...
import type { Notificacion, ResumenAlertasProactivas } from "@/types/entities";
...
export async function getNotificaciones(limit = 10): Promise<Notificacion[]> {
  ...
  for (const s of seguimientosRes.data ?? []) {
    ...
    items.push({
      ...
      descripcion: pres?.leads?.nombre ? `Lead: ${pres.leads.nombre}` : undefined,
  ...
  for (const i of instalacionesRes.data ?? []) {
    items.push({
      id: `inst-${i.id}`,
      tipo: "instalacion",
      titulo: `Instalacion: ${i.nombre ?? "Proyecto"}`,
  ...
  for (const r of revisionesRes.data ?? []) {
    ...
    items.push({
      id: `rev-${r.id}`,
      tipo: "revision",
      titulo: `Revision ${r.tipo} pendiente`,
      descripcion: cli?.leads?.nombre ? `Cliente: ${cli.leads.nombre}` : undefined,
